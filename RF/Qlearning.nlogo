...
to setup ;--this builds the maze that the walker has to learn to get through
  ...
  ask patches with [pxcor =  max-pxcor or pxcor = min-pxcor or pycor = max-pycor or pycor = min-pycor][set pcolor blue]
  ...
  ask patch -2 -3 [
    set pcolor blue
    ask one-of neighbors4 [set pcolor blue]]
  ask one-of patches with [pycor = -1 and pxcor != min-pxcor and pxcor != max-pxcor]
...
to go
  ...
  while [episode <= num-episodes]
    ...
    pen-down
      ...
      plot ave-reward
      print sentence "--------episode " episode
